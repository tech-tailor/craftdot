    ...
    return `
      <div class="artisan-board">
          <div class="container1">
            <img class="profile-picture" src="${artisanData.profile_picture}" alt="${artisanData.first_name}">
            <p class="name">${artisanData.first_name} </p><br>
            <p class="price">${artisan.pricing}/hr</p>
            <p class="reviews"> 4.8 (50 reviews)</p>
            <p class="price"> 300 ${serviceType} tasks</p>
            <p class="state">State: ${artisanData.state}</p>
            <p class="experience">Experience: ${artisan.experience}</p>  
          </div>
          
          <button type="button" value="${ artisan.user_id }" class="btn btn-primary schedule" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
              Select and Continue
          </button>
      </div>
    `;
